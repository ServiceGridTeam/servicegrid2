...
export function WorkerRouteSidebar({
  ...
  // Get workers with their route info
    ...
    <div className="w-72 border-r bg-background shrink-0 flex flex-col">
      ...
      <ScrollArea className="flex-1">
        <div className="p-2 space-y-1">
          {workersWithRoutes.map(({ member, route, color, jobCount, distanceMeters, durationSeconds, maxJobs }) => {
            ...
            return (
              <button
                key={member.id}
                onClick={() => onSelectWorker(isSelected ? null : member.id)}
